...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "kategoria bólu"
      decision: "tarcie w przepływie pracy — istniejące apki fiszek wymagają rąk/wzroku, blokując równoległą aktywność fizyczną"
    - topic: "główna persona MVP"
      decision: "developer jako użytkownik — budujesz dla siebie"
    - topic: "obecny workaround"
      decision: "czas aktywności fizycznej jest zmarnowany z perspektywy nauki; użytkownik po prostu nie uczy się podczas biegania"
    - topic: "uwierzytelnianie"
      decision: "OAuth Google w MVP; Facebook i Apple Sign-In planowane w v2"
    - topic: "role użytkowników"
      decision: "jeden typ użytkownika, płaski model; brak administratora w MVP"
  frs_drafted: 15
...
- Tryb nauki działa bez dostępu do internetu: fiszki muszą być dostępne lokalnie na urządzeniu (zsynchronizowane przed sesją), a TTS używa lokalnych głosów Androida (offline voice pack).
...
- Brak trybu nauki na Web — aplikacja Web służy wyłącznie do zarządzania fiszkami (tworzenie, edycja, przeglądanie); nauka audio działa tylko na Android.
- Brak aplikacji na iPhone w MVP — tylko Android; iPhone to przyszły etap po ustabilizowaniu MVP.
...
Potwierdzono dnia 2026-05-22: 4-tygodniowe MVP po godzinach wymaga stałego zaangażowania (wieczory i weekendy); użytkownik zaakceptował świadomie wybierając zakres 3-4 tygodnie.
...
2. **Synchronizacja fiszek offline** — kiedy dokładnie aplikacja synchronizuje fiszki z chmury na urządzenie? (przy każdym uruchomieniu? przy starcie trybu nauki? w tle?) Do ustalenia w projekcie architektonicznym.
3. **Liczba powtórzeń tekstu EN** — domyślnie 3, ale czy powinna być konfigurowalna? Na MVP hardkodowane 3, konfiguracja w v2.

## Forward: tech-stack

Uwagi do dalszego wyboru stosu (poza zakresem PRD):
- Platforma: Android (must-have MVP) + Web (zarządzanie fiszkami)
- TTS: systemowy Android TTS (offline voice)
- Background audio: Foreground Service + MediaSession API
- Stos wybrany: Kotlin Multiplatform (Kotlin 2.3.21 + Compose Multiplatform 1.11.0) — moduły androidApp, shared, webApp; iOS cel dostępny w szablonie KMP, nie aktywowany w MVP; backend: Rust + Actix-web
