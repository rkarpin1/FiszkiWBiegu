...
- **Backend / API:** obecny — Actix-web 4.13 (v0.1.9); auth JWT (Google OAuth, 30 dni); endpointy: GET /info; POST /auth/login, GET /auth/me; GET|POST /collections, PUT|DELETE /collections/{id}; GET|POST /collections/{id}/flashcards, GET /collections/{id}/learning, POST /collections/{id}/learning/complete; PUT|DELETE /flashcards/{id}; POST /translate (proxy do Azure Translator, 503 gdy nieskonfigurowane); POST /deploy (self-update binarium, auth X-Deploy-Key); walidacja: kody języka (pl/en/de/es/fr/it), source ≠ target, nazwa niepusta (422); pełna specyfikacja: `context/docs/openapi.yaml`. Testy integracyjne w `apps/backend/tests/` (testcontainers + Docker Postgres)
- **Dane:** obecny — sqlx + PostgreSQL (backend); migracje 001–011 (następna: 012): `001_init`, `002_add_users`, `003_add_languages`, `004_add_description`, `005_add_user_profile`, `006_add_collection_tracking`, `007_rename_flashcard_columns` (polish_text→source_text, english_text→target_text; S-06), `008_add_srs_level`, `009_add_last_studied_at`, `010_add_total_study_minutes`, `011_drop_collection_progress` (kolumna `progress` usunięta — liczona per zapytanie ze średniego zaniku SRS); schemat: `users(google_id, email, display_name, streak_days)`, `collections(user_id, name, description, source_language, target_language, last_studied, total_study_minutes)` — `flashcard_count` i `progress` liczone per zapytanie, nie kolumny, `flashcards(collection_id, source_text, target_text, position, srs_level, last_studied_at)`; pełny schemat: `context/docs/database.md`; lokalny cache fiszek offline obecny (F-01, Android); token JWT persistowany przez multiplatform-settings
...
### Zmiany poza slice'ami roadmapy (ad-hoc: bugfixy, refaktory, chore)

Zmiany zarchiwizowane bez odpowiadającego im slice'a w roadmapie — wykonane doraźnie, ujęte tu dla pełności rejestru.

- **s-02a: Automatyczne wylogowanie przy wygaśnięciu tokenu (Ktor 401 interceptor)** — Zarchiwizowano 2026-05-29 → `context/archive/2026-05-27-s-02a/`. Lekcja: —.
- **collection-language-select: Wybór języka źródłowego i docelowego przy tworzeniu/edycji kolekcji (frontend + backend)** — Zarchiwizowano 2026-05-29 → `context/archive/2026-05-29-collection-language-select/`. Lekcja: —.
- **project-structure-cleanup: Porządki w strukturze projektu** — Zarchiwizowano 2026-05-29 → `context/archive/2026-05-29-project-structure-cleanup/`. Lekcja: —.
- **pull-to-refresh: Odświeżanie gestem przeciągnięcia na liście kolekcji i ekranie kolekcji (Material3 PullToRefreshBox)** — Zarchiwizowano 2026-06-25 → `context/archive/2026-06-25-pull-to-refresh/`. Lekcja: —.
- **seed-new-user: Kopiowanie kolekcji i fiszek z konta-szablonu (`rkarpin1@gmail.com`) dla nowo zarejestrowanego użytkownika** — Zarchiwizowano 2026-06-25 → `context/archive/2026-06-25-seed-new-user/`. Lekcja: —.
- **collection-refresh-hang: (bugfix, research-only) diagnoza wiecznego spinnera po dodaniu fiszki — brak `HttpTimeout` w kliencie Ktor; timeouty HTTP wdrożone i wykorzystane w pull-to-refresh** — Zarchiwizowano 2026-06-25 → `context/archive/2026-06-25-collection-refresh-hang/`. Lekcja: —.
